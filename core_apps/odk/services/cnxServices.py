...
from core_apps.odk.models import (
    ODKAuditLogs,
    ODKProjectPermissions,
    ODKProjects,
    ODKUserSessions,
)
...
class ODKCentralService:
    ...
    def _get_or_create_token(self) -> str:
        ...
        try:
            ...
            expires_at = timezone.now() + timedelta(
                hours=23
            )  # 23h pour éviter les expirations
            ...
            logger.info(
                f"Authentification ODK réussie pour le compte {account['id']} (threads: {thread_id})"
            )
            return token
            ...
            thread_id = threading.current_thread().ident
            logger.error(
                f"Échec d'authentification ODK pour le compte {account['id']}: {e} (thread: {thread_id})"
            )
            raise
        ...
        for attempt in range(max_retries):
            try:
                ...
                if e.response.status_code == 401:  # Token expiré
                    logger.warning(
                        f"Token expiré pour le compte {self.current_account['id']}, rafraîchissement..."
                    )
                ...
                logger.error(
                    f"Erreur HTTP {status_code} lors de la requête à ODK Central: {e}"
                )
        ...
        try:
            ...
            for project_data in all_projects:
                if self._user_can_access_project_id(project_data["id"]):
                    ...
                    try:
                        django_project = ODKProjects.objects.get(
                            odk_id=project_data["id"]
                        )
                        permission = django_project.get_user_permission(
                            self.django_user
                        )

                        project_data["django_permission"] = (
                            permission.permission_level if permission else None
                        )
    ...
    def create_project(self, project_data: Dict) -> Dict:
        ...
        try:
            if (
                not self.django_user.profile.odk_role
    ...
    def _user_can_access_project_id(self, project_id: int) -> bool:
        ...
        if (
            self.django_user.profile.odk_role
            == self.django_user.profile.ODKRole.ADMINISTRATOR
        ):
            return True
        ...
        try:
            ...
            try:
                ...
                ODKProjectPermission.objects.get(user=self.django_user, project=project)
                return True
    ...
    def _log_action(
        ...
        try:
            ...
            logger.error(
                f"Erreur lors de l'enregistrement dans le journal d'audit: {e}"
            )
