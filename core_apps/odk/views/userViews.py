            ...
            with ODKAppUserService(request.user, request=request) as app_user_service:
                try:
                    ...
                    for app_user in app_users:
                        try:
                            server_url = getattr(settings, "ODK_CENTRAL_URL", "https://odk.insuco.net")
    ...
    def get(self, request, project_id:int, form_id:str):
        ...
        try:
            with ODKAppUserService(request.user, request=request) as app_user_service:
                ...
                if error_response:
                    return error_response
                app_users = app_user_service.list_forms_app_users(odk_project_id, form_id)
                return Response(
                    {"count": len(app_users), "results": app_users},
                    status=status.HTTP_200_OK,
                )
            ...
            with ODKCentralService(request.user, request=request) as odk_service:
                ...
                for app_user in app_users:
                    ...
                    for form in forms:
                        ...
                        assigned_users = odk_service.list_forms_app_users(odk_project_id, form_id)
